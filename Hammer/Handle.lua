...
local HammerTrail = Hammer:WaitForChild("HammerTrail")
...
weaponRemote.OnServerEvent:Connect(function(Player, Argument, Time)
	...
	if Argument == "Equipped" then
		attachment1.Position = Vector3.new(0,.78, .5)
		attachment2.Position = Vector3.new(0,.78, -.5)
		...
		Hammer:SetPrimaryPartCFrame(rightArm.CFrame * CFrame.Angles(math.rad(270),math.rad(180),math.rad(90)) * CFrame.new(-1.3,0,.08))
	...
	if Argument == "Unequipped" then
		...
		NewWeld.Part0 = torso 
		Hammer:WaitForChild("Weld"):Destroy()
		Hammer:SetPrimaryPartCFrame(torso.CFrame * CFrame.Angles(math.rad(90),math.rad(198),math.rad(180)) * CFrame.new(-0.34,-0.45,0.385))
		NewWeld.Parent = Hammer
	end	
end)
